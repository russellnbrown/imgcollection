...
bool icUtils::IsImageFile(fs::path fileStr)
{
...
void icUtils::Replace(string& source, string const& find, string const& replace)
{
...
bool icUtils::GetImageInfo(ImageInfoSPtr& ii)
{
	try
	{
		...
		ii->size = (int32_t)pos;
		//ii->thumb = make_unique<int8_t[]>(TNMEM);
		ii->bytes = new int8_t[ii->size];
		ifs.seekg(0, ios::beg);
		ifs.read((char*)ii->bytes, ii->size);
		ifs.close();

		ii->crc = icUtils::GetHash(ii->bytes, ii->size);
		...
		for (int r = 0; r < pw; r++)
		{
			...
			for (int c = 0; c < pw; c++)
			{
				...
				//logger::raw(buf);
				ii->thumb.push_back(sl[tb + 2]);
				ii->thumb.push_back(sl[tb + 1]);
				ii->thumb.push_back(sl[tb + 0]);
//				ii->thumb[wb++] = sl[tb + 2];
	//			ii->thumb[wb++] = sl[tb + 1];
		//		ii->thumb[wb++] = sl[tb + 0];
				tb += 3;
...
void icUtils::PrintThumb(const char* txt, ThumbVec& t)
{
	printf("%s : Pixels:-\n", txt);
	int ix = 0;
...
double icUtils::GetCloseness(ThumbVec& srch, ThumbVec& cand, SearchAlgo scanType)
{

	double td = 0.0;

	if (scanType == Assembler)
		return GetAsmCloseness(srch.data(), cand.data());
...
double icUtils::GetAsmCloseness(int8_t* i1, int8_t* i2)
{
	uint32_t diff = 0;
